...
elseif strcmp(ms,'true1') %homogeneous, nonzero Dirichlet BC's 
    kt = c*sqrt((kx^2) + (ky^2)); 
    ue = @(x,y,t) sin(kx*x).*sin(ky*y).*sin(kt*t) + 0.5;
    ut = @(x,y,t) kt*sin(kx*x).*sin(ky*y).*cos(kt*t);
    f  = @(x,y,t) 0.5;
    gax = @(y,t) ue(ax,y,t);
    gbx = @(y,t) ue(bx,y,t);
...
for m = 1:numResolutions
    ...
    for n = 2:Nt
        ...
        rhs = getRHS(un,unm1,J1,J2,I1,I2,Ngx,Ngy,dt,fn,eta,dpmx,dpmy,c);
        %rhs(I,J) = getRHS(un,unm1,J1,J2,I1,I2,Ngx,Ngy,dt,fn,eta,dpmx,dpmy,c);
